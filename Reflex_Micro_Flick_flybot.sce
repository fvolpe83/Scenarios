Name=Reflex Micro++ Flick flybot
PlayerCharacters=Clicker;react
...
WeaponHeroTag=
DifficultyTag=3
AuthorsTag=based on Reflex Flick by KovaaK (edited by flexxis), bot from Fuglaa, glued together by Faleene
BlockHitMarkers=true
BlockHitSounds=false
BlockMissSounds=true
BlockFCT=true
Description=Even smaller Micro distance flick training. Train the most consistent form of flicking at a target - micro flicking - which usually follows a bigger, more chunky flick.Try your best to hit each target consciously for maximal learning effect, even if having to go slow first (no self destruct of target).The faster you are, the higher you potentially can score - a target respawns instantly. Be aware that score is accuracy weighted! Moving target
GameVersion=1.0.6
