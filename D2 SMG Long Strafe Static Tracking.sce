Name=D2 SMG Long Strafe Static Tracking
PlayerCharacters=Quaker;Quaker enemy long strafe
...
BlockFCT=false
Description=This exercise focuses on tracking a medium range target moving in one direction for an extended period. Static tracking will be utilized in this exercise. Which means your reticle will be fixed while you and the target are moving. Therefore the majority of your aiming will be from your keyboard with only minor adjustments with the mouse. Try to match your speed with the target and only ADS to fire when the target is completely centered in the screen. Apply model aiming and shoot the head for a more efficient kill.To get a more exact and visual explanation please checkout my Youtube Channel at https://www.youtube.com/channel/UCeWR  or search HazedNConfused.
GameVersion=2.0.2.0
